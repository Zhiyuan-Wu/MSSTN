...
def IDW2Image(filename='./data/bjdata2_PM25.npy'):
    data = np.load(filename)[:,:,:-2]
    SiteLocation = pd.read_csv('./data/BJSiteLocation.csv').sort_values('id')
    station_list = list(SiteLocation[['id']].to_dict()['id'].values())
    location = SiteLocation[['lon','lat']].as_matrix()
    num_nodes = len(station_list)
    ...
    for i in range(height):
        for j in range(width):
            target.append([115.0+j*0.1,39.0+i*0.1])
    target = np.array(target)
    w = []
    for i in range(location.shape[0]):
        d = np.exp(-np.sum(np.square(location[i]-target),1)/0.1)
        w.append(d)
    w = np.array(w)
    w = w/np.sum(w,0)
    
    data = data.transpose([0,2,1]).reshape([time_length*channel_num,num_nodes])
    result = np.matmul(data,w).reshape([time_length,channel_num,height,width])
    result = result.transpose([0,2,3,1])
    np.save(filename+'.img',result)
    return result
