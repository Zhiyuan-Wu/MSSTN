...
class ReiWa(object):
    ...
    def default_model(self,args):
        ...
        self.input_0 = tf.placeholder(tf.float32, shape=(args['num_nodes_0'], args['seq_len'], args['input_dim_0']), name='input_0')
        self.spatial_img_0 = tf.placeholder(tf.float32, shape=(args['seq_len'], args['img_height'], args['img_width'], args['spatial_feature_dim_0']), name='input_0')
        spatial_feature = self.CNN(self.spatial_img_0,'CNN_0',args)
        print('extracted spatial_feature dim: ',spatial_feature.shape[-1].value)
        spatial_features = tf.stack([spatial_feature for _ in range(args['num_nodes_0'])],0)
        self.pred_0, self.loss_0, self.loss_mse_0 = self.DCN(tf.concat([self.input_0,spatial_features],axis=2), 'DCN_0', args)
    ...
    def CNN(self, input, name, args):
        batch_size = input.shape[0].value
        n_input = input.shape[-1].value
        n_output = 32
        layers = 3
        with tf.variable_scope(name) as scope:
            h = input
            for i in range(layers):
                w = tf.get_variable('conv'+str(i),[3, 3, h.shape[-1].value, n_output],initializer=tf.contrib.layers.xavier_initializer())
                b = tf.get_variable('conv_bias'+str(i),[n_output],initializer=tf.contrib.layers.xavier_initializer())
                h = tf.nn.conv2d(h,filter=w,strides=[1,1,1,1],padding='SAME')+b
                h = tf.nn.relu(h)
