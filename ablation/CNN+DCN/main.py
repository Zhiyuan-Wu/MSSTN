        ...
        if len(argv)==1 or 'train' in argv[1]:
            ...
            for epoch in range(config['epoch_num']):
                ...
                for batch_num in range(data.tr_batch_num):
                    batch_mid,batch_set,batch_spatial_img = data.tr_get_batch()
                    feed_dict = {model.learning_rate:lr,model.input_0:batch_set[0],model.spatial_img_0:batch_spatial_img}
                ...
                if (epoch+1)%config['test_every_n_epochs'] == 0:
                    ...
                    for batch_num in range(data.te_batch_num):
                        batch_mid,batch_set,batch_spatial_img = data.te_get_batch()
                        feed_dict = {model.learning_rate:lr,model.input_0:batch_set[0],model.spatial_img_0:batch_spatial_img}
