        ...
        if len(argv)==1 or 'train' in argv[1]:
            ...
            for epoch in range(config['epoch_num']):
                ...
                for batch_num in range(data.tr_batch_num):
                    batch_mid,batch_set,batch_spatial_feature = data.tr_get_batch()
                    feed_dict = {model.learning_rate:lr,model.input_0:batch_set[0],model.spatial_feature_0:batch_spatial_feature}
                ...
                if (epoch+1)%config['test_every_n_epochs'] == 0:
                    ...
                    for batch_num in range(data.te_batch_num):
                        batch_mid,batch_set,batch_spatial_feature = data.te_get_batch()
                        feed_dict = {model.learning_rate:lr,model.input_0:batch_set[0],model.spatial_feature_0:batch_spatial_feature}
                        ls1 = sess.run(model.loss,feed_dict)
                        ls3 = sess.run(model.loss_mse_0,feed_dict)
                        test_ls.append(ls1)
                        test_ls2.append(0)
                        test_ls3.append(ls3)
                    test_ls = np.mean(test_ls)*500
                    test_ls2 = np.mean(test_ls2)*500
                    test_ls3 = np.sqrt(np.mean(test_ls3))*500
