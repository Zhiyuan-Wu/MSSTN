...
class dataset(object):
    def __init__(self,args):
        ...
        self.tr = []
        self.te = []
        self.spatial_feature = np.load(args['spatial_feature_path'])
        self.tr_spatial_feature = self.spatial_feature[:81*90+1]
        self.te_spatial_feature = self.spatial_feature[81*90+1:81*117+2]
        self.tr_spatial_feature = np.transpose(self.tr_spatial_feature,(1,0,2))
        self.te_spatial_feature = np.transpose(self.te_spatial_feature,(1,0,2))
    ...
    def tr_get_batch(self,id=None):
        ...
        batch_spatial_feature = self.tr_spatial_feature[:,idx_start:idx_end+1]
        batch_set = []
        for i in range(len(self.tr)):
            batch_set.append(self.tr[i][:,idx_start:idx_end+1])
        
        return batch_mid,batch_set,batch_spatial_feature
    ...
    def te_get_batch(self,id=None):
        ...
        batch_spatial_feature = self.te_spatial_feature[:,idx_start:idx_end+1]
        batch_set = []
...
if __name__=='__main__':
    ...
    ZYFeature('./data/PM2.5/bjPM2.5.npy')
    _debug = np.array([2,3,3])